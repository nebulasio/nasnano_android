...
import kotlinx.android.synthetic.nas_nano.activity_launch_wallet_password_verify.*

class LaunchWalletPasswordVerifyActivity : BaseActivity(), FingerprintVerifyDialog.ActionListener {

    override fun initView() {}

    companion object {
        fun launch(activity: Activity, requestCode: Int) {
            activity.startActivityForResult(Intent(activity, LaunchWalletPasswordVerifyActivity::class.java), requestCode)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        titleTV.text = getString(R.string.action_to_verify_wallet_password_short)
        initPasswordVerify()
    }
    ...
    private fun initPasswordVerify() {
        ...
        tv_action_verify_fingerprint.setOnClickListener {
    ...
    private fun verifyPassword(pwd: String) {
        ...
        if (selectedWalletWrapper.wallet.isComplexPwd) {
            ...
            passwordInputDelegate.removeFocus()
        }
        showLoadingLayout()
    ...
    private fun cancelFingerprintVerify() {
        cancellationSignal?.apply {
            if (!isCanceled) {
                try {
                    cancel()
    ...
    inner class WalletWrapper(val wallet: Wallet, val index: Int) : NasBottomListDialog.ItemWrapper<Wallet> {
        override fun getDisplayText(): String {
            val defaultPaymentWalletId = Util.getDefaultPaymentWallet(this@LaunchWalletPasswordVerifyActivity)
