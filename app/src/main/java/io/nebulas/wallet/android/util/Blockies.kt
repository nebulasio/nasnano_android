...
object Blockies {
    ...
    /**
     * generate the blocky icon.ps:account need use the lowercase!
     */
    @JvmOverloads
    fun createIcon(address: String, scale: Int = 16, circleImage: Boolean = false): Bitmap {
        seedrand(address)
        ...
        return createCanvas(imgdata, color, bgColor, spotColor, scale, circleImage)
    }

    private fun createCanvas(imgData: DoubleArray, color: HSL, bgcolor: HSL, spotcolor: HSL, scale: Int, circleImage: Boolean): Bitmap {
        ...
        return getCroppedBitmap(bmp, circleImage)
    }
    ...
    fun getCroppedBitmap(bitmap: Bitmap, circleImage: Boolean): Bitmap {
        ...
        if (circleImage) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                canvas.drawRoundRect(0f, 0f, bitmap.width.toFloat(), bitmap.height.toFloat(), 5f, 5f, paint)
            } else {
